...
/**
 * Returns true if the `cwd` looks like a leanspec project (skills are installed).
...
async function planSkillFile(cwd, rel, dryRun) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      return { target, kind: 'skill', action: 'missing-target', detail: 'not installed (run `leanspec init` first)' };
    }
    throw err;
  }
...
async function planAgentsSection(cwd, headingLine, dryRun) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      return { target, kind: 'agents-section', action: 'missing-target', detail: 'AGENTS.md not found (run `leanspec init` first)' };
    }
    throw err;
  }
  ...
  if (!currentSection) {
    return {
      ...
      detail: `heading "${headingLine}" not found in AGENTS.md — your file may predate this section. Re-run \`leanspec init --overwrite\` (with a backup) or copy the section manually from the upstream template.`,
    };
  }
