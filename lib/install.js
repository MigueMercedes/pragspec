  ...
  for (const rel of filtered) {
    ...
    if (rel === path.join('specs', 'templates', 'feature.spec.md') && validExts.length > 0) {
      ...
      if (optIdx !== -1) {
        ...
        const extBlock =
          `<!-- Extensions enabled: ${validExts.join(', ')} (configured via pragspec init) -->\n\n` +
          extBlocks.join('\n\n') +
          '\n\n---\n\n';
 ...
 * Idempotent: skips if any known marker line already exists. Legacy markers
 * (`# Added by claude-sdd`, `# Added by leanspec`) are honored for
 * backward-compat — users who installed pre-rename(s) do not get a duplicate
 * block on next `init`.
 * @param {string} cwd
