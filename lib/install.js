  ...
  for (const rel of filtered) {
    ...
    if (rel === path.join('specs', 'templates', 'feature.spec.md') && validExts.length > 0) {
      ...
      if (optIdx !== -1) {
        ...
        const extBlock =
          `<!-- Extensions enabled: ${validExts.join(', ')} (configured via leanspec init) -->\n\n` +
          extBlocks.join('\n\n') +
          '\n\n---\n\n';
 ...
 * Idempotent: skips if any known marker line already exists. The legacy
 * `# Added by claude-sdd` marker is honored for backward-compat — users who
 * installed pre-rename do not get a duplicate block on next `init`.
 * @param {string} cwd
...
export async function appendGitignore(cwd) {
  ...
  const knownMarkers = ['# Added by leanspec', '# Added by claude-sdd'];
  ...
  if (knownMarkers.some((m) => existing.includes(m))) {
